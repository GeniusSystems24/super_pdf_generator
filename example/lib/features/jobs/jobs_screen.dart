...
class _JobsScreenState extends State<JobsScreen> {
          ...
          children: [
                ...
                children: [
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                        border: Border(bottom: BorderSide(color: gl.border))),
                    child: Row(children: [
                      Text('Job Queue',
                      ...
                      GlGhostButton(
                        ...
                        onPressed: () {
                          ...
                          _paused
                              ? widget.client.jobs.pauseQueue()
                              : widget.client.jobs.resumeQueue();
                        },
                      ),
                      ...
                      GlGhostButton(
                          ...
                          onPressed: widget.client.jobs.clearCompleted),
                    ]),
                  ),
                  // stats strip
                  Container(
                  ...
                  Container(
                    ...
                    child: Row(children: [
                      ...
                      Expanded(
                          ...
                          child: Text('STATUS',
                              style:
                                  GlType.label(context).copyWith(fontSize: 9.5),
                  ...
                  Expanded(
                    child: rows.isEmpty
                        ? Center(
                            child: Text(
                                'No jobs yet — generate a batch to populate the queue.',
                                style: GlType.body(context,
                                    size: 12.5, color: gl.fg3)))
                        : ListView.separated(
                            ...
                            itemBuilder: (context, i) => _JobRow(
                              ...
                              onTap: () =>
                                  setState(() => _selectedId = rows[i].id),
                            ),
                          ),
                  ),
                ],
            ...
            if (selected != null)
              Container(
                width: 250,
                decoration: BoxDecoration(
                    border: Border(left: BorderSide(color: gl.border)),
                    color: gl.surface),
                child: _Drawer(
                  job: selected,
  ...
  Widget _stat(BuildContext context, String v, String l, Color c) => Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(v,
              style: GlType.mono(context,
                  size: 17, color: c, weight: FontWeight.w700)),
          Text(l,
              style: GlType.body(context, size: 9.5, color: context.gl.fg3)),
        ]),
      );
}
...
class _JobRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final progress = job.status is JobRunning
        ? (job.status as JobRunning).progress
        : (job.status is JobCompleted ? 1.0 : 0.0);
    return Material(
      color: selected
          ? gl.hover
          : (isFail ? gl.danger.withOpacity(0.05) : Colors.transparent),
      child: InkWell(
        onTap: onTap,
        child: Padding(
          ...
          child: Row(children: [
            Expanded(
              ...
              child: Column(
                  ...
                  children: [
                    Text(job.label,
                        style: GlType.mono(context, size: 11.5, color: gl.fg1),
                        overflow: TextOverflow.ellipsis),
                    Text(
                        isFail
                            ? 'failure · tap for details'
                            : (job.batchId != null ? 'batch item' : 'generate'),
                        style: GlType.body(context,
                            size: 9.5, color: isFail ? gl.danger : gl.fg4)),
                  ]),
            ),
            Expanded(flex: 2, child: _priority(context, job.priority)),
            Expanded(
                flex: 2,
                child: Text('${job.retries}/${job.maxRetries}',
                    style: GlType.mono(context,
                        size: 11,
                        color: job.retries > 0 ? gl.orange : gl.fg3))),
            Expanded(
              flex: 3,
              child: Padding(
                ...
                child: ClipRRect(
                  ...
                  child: LinearProgressIndicator(
                      value: progress,
                      minHeight: 5,
                      backgroundColor: gl.input,
                      color: isFail ? gl.danger : gl.accent),
                ),
              ),
            ),
            Expanded(
                ...
                child: Align(
                    ...
                    child: _statusPill(context, job.status))),
          ]),
        ),
      ),
    );
  ...
  Widget _priority(BuildContext context, PdfJobPriority p) {
    ...
    return Align(
        ...
        child: GlStatusPill(label, color: color));
  }
...
class _Drawer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final gl = context.gl;
    final failure =
        job.status is JobFailed ? (job.status as JobFailed).error : null;
    return Column(
      ...
      children: [
          ...
          child: Row(children: [
            ...
            Expanded(
                child: Text('Job details',
                    style: GlType.body(context,
                        size: 13, weight: FontWeight.w700, color: gl.fg1))),
            GestureDetector(
                onTap: onClose,
                child: Icon(Icons.close_rounded, size: 16, color: gl.fg3)),
          ]),
        ),
        Expanded(
          child: ListView(
            ...
            children: [
              ...
              Text('${job.id} · ${job.priority.name}',
                  style: GlType.mono(context, size: 10, color: gl.fg4)),
              ...
              if (job.duration != null)
                _kv(context, 'Duration', '${job.duration!.inMilliseconds}ms'),
              if (failure is PdfFailure) ...[
                const SizedBox(height: 14),
                Container(
                  padding: const EdgeInsets.all(12),
                  decoration: BoxDecoration(
                      color: gl.danger.withOpacity(0.07),
                      borderRadius: const BorderRadius.all(GlRadius.md),
                      border: Border.all(color: gl.danger.withOpacity(0.26))),
                  child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(failure.code,
                        ...
                        Text(failure.message,
                            style: GlType.body(context, size: 11, color: gl.fg2)
                                .copyWith(height: 1.5)),
                        if (failure.recovery != null) ...[
                          const SizedBox(height: 8),
                          Text('RECOVERY',
                              style: GlType.label(context, color: gl.fg4)
                                  .copyWith(fontSize: 8.5)),
                          const SizedBox(height: 3),
                          Text(failure.recovery!,
                              style:
                                  GlType.body(context, size: 11, color: gl.fg2)
                                      .copyWith(height: 1.5)),
                        ],
                      ]),
                ),
        ...
        Container(
          padding: const EdgeInsets.all(12),
          decoration:
              BoxDecoration(border: Border(top: BorderSide(color: gl.border))),
          child: Row(children: [
            Expanded(
            ...
            const SizedBox(width: 8),
            Expanded(
                child: GlGhostButton(label: 'Cancel', onPressed: onCancel)),
          ]),
        ),
      ],
    );
  ...
  Widget _kv(BuildContext context, String k, String v) => Padding(
        ...
        child:
          ...
          Text(k,
              style: GlType.body(context, size: 11.5, color: context.gl.fg3)),
